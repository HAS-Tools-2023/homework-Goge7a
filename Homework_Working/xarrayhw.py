...
# TODO: Your code here
attrs = ds.attrs  # Extract attributes
author = attrs.get("author", "Author information not available")  # Get the author attribute
...
for var in ds: 
    variable = ds[var]
    description = variable.attrs.get("description", "Description not available")
    units = variable.attrs.get("units", "Units not available")
...
# Display the subset of data for the first day
print(first_ds)
...
# from the full `ds`
#TODO: Your code here
subset_ds = ds.sel(lat=slice(ds.lat[0], ds.lat[29]), lon=slice(ds.lon[19], ds.lon[39]))
...
# Display the spatial mean dataset
print(spatial_mean_ds)
...
correlation_matrix = np.corrcoef(
    spatial_mean_ds['potential_evapotranspiration'].values.flatten(),
    spatial_mean_ds['mean_vapor_pressure_deficit'].values.flatten()
)
...
correlation
correlation = xr.corr(
    coarse_ds['potential_evapotranspiration'],
    coarse_ds['mean_vapor_pressure_deficit'],
    dim='day'
)
